...
## v0.11.0 AI 선택 수동 브러시 검증

- 단위 테스트 45개 통과. AI 지도 위에 더하기·지우기 획을 순서대로 합성하고, 반전/강도는 합성 후 적용합니다. 원래 AI 지도 불변성, 획별 크기/부드러움 유지, 회전·뒤집기·자르기, JSON 왕복, 잘못되거나 과도한 경로 거부를 검증합니다.
- 피사체 통합 테스트에 실제 더하기·지우기 드래그를 추가했습니다. 피사체의 지운 부분은 원래 픽셀로 돌아오고 추가 영역만 보정되며, 획별 실행 취소/다시 실행·Esc 취소·수동 수정 초기화와 복원·AI 재선택 시 수동 경로 유지·프로젝트 v5 저장과 재열기·자동 복원·HDR 출력 및 재입력 인식을 확인합니다.
- 수동 경로는 최대 128획/4,096점으로 제한합니다. 선택 지도와 별도로 보존하므로 AI 재인식 시 손실되지 않으며 원본 사진의 픽셀 정밀도도 바꾸지 않습니다.

macOS v0.11.0 패키지에서 피사체 수동 수정/HDR 통합 테스트, 기존 브러시·그라디언트 테스트, 전체 편집 UI 회귀 테스트를 통과했습니다. Windows x64 NSIS 설치 파일도 생성했습니다.

최종 코드 `db5adce`의 [Windows 검증](https://github.com/murikubo/HinanaStudioImage/actions/runs/35244129108)에서 단위 45개, 설치 파일 생성, 실제 EXE의 전체 UI/P3/HDR/기존 마스크 테스트, AI 선택 후 수동 브러시 더하기·지우기와 저장/복원 테스트, 상단바 및 RAW 현상·프로젝트 복원이 모두 통과했습니다.

## v0.11.1 마스크 도구 구분

새 마스크 생성 메뉴와 현재 마스크 수정 도구를 분리했습니다. 실제 Electron 피사체 통합 테스트에서 브러시로 전환해도 마스크 목록이 하나로 유지되는지, 수정 도구 하나만 활성화되는지, 브러시 모드에서 AI 지점이 숨겨지는지 확인했습니다. AI 포함/제외, 수동 더하기/지우기, 실행 취소, 초기화, 저장/재열기, HDR 출력/재입력 및 기존 원형·브러시 통합 테스트가 통과했습니다.
