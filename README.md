...
- 피사체 선택 후 **브러시로 더하기 / 브러시로 지우기**를 선택해 사진 위를 드래그합니다. 브러시 반경과 경계 부드러움을 조절할 수 있고, 사진 위 원으로 반경을 확인합니다. Alt/Option을 누른 채 획을 시작하면 지우기입니다.
- 각 획은 그릴 때의 크기·부드러움을 유지합니다. 실행 취소/다시 실행은 획 단위로 동작하며, Esc는 현재 그리고 있는 획만 취소합니다. **수동 수정 초기화**로 AI 선택 결과만 남길 수 있고 이 초기화도 실행 취소할 수 있습니다.
- **AI 클릭**으로 돌아가 선택점을 추가해도 수동 수정은 유지됩니다. `선택 다시 시작`은 AI 선택점과 수동 수정을 모두 비웁니다. 반전을 켰을 때도 브러시는 반전 전 선택 영역을 수정합니다.
- 마스크당 128획·총 4,096점을 지원합니다. 수동 경로는 원본 좌표로 저장되어 자르기·회전·뒤집기, 고정밀/HDR 출력에 함께 적용됩니다. 프로젝트는 v5로 저장하며 기존 v1–v4 파일도 열 수 있습니다. v5 파일은 이전 앱에서 열 수 없습니다.

## v0.10.0 피사체 선택

- **마스크 → 피사체 선택 → 사진의 피사체 클릭**으로 영역을 만듭니다. **선택에 추가 / 선택에서 제외**를 고른 후 다시 클릭해 보완합니다. Alt/Option 클릭은 제외이며, 마스크당 최대 32개 선택점을 지원합니다.
- 빨간색은 선택 영역 안내입니다. **로컬 노출·대비·채도·색온도**를 조절하면 그 영역의 사진이 바뀝니다. `선택 영역 표시`를 끄면 보정 결과를 가림 없이 볼 수 있습니다. 반전하면 배경을 보정할 수 있습니다.
- 첫 클릭은 인식 계산이 필요하며, 같은 사진의 추가 클릭은 저장된 특징을 재사용합니다. 취소하면 이전 선택 영역을 유지합니다. 사진·마스크·탭 전환 또는 실행 취소도 진행 중인 인식을 취소합니다.
- SlimSAM 모델은 앱에 포함되어 오프라인으로 실행됩니다. 사진은 외부로 전송하지 않습니다. CPU 기반이며 첫 실행 시간은 컴퓨터에 따라 달라집니다. 피사체를 자동으로 찾는 원클릭 전경 감지와는 달리 **대상을 클릭해야 합니다**. 머리카락·반투명 물체·작은 피사체의 경계에는 한계가 있습니다. 최대 1024px 선택 지도를 원본 좌표에 맞춰 적용하며, 원본 사진의 정밀도는 낮추지 않습니다.
- 선택 지도와 클릭 지점은 `.hinanaimage` 및 자동 저장에 보존됩니다. 프로젝트 형식 v4로 저장하며 v1–v3도 불러옵니다. 새 프로젝트는 이전 앱에서 열 수 없습니다. 브라우저 전용 실행에서는 AI 선택 버튼이 비활성화됩니다.
- Windows의 AI 인식에는 [ONNX Runtime이 요구하는 Visual C++ 런타임](https://onnxruntime.ai/docs/install/)이 필요합니다. 엔진 시작 오류가 나면 Microsoft의 최신 x64 Visual C++ 재배포 패키지를 설치해 주세요.
- 빌드 시 `scripts/fetch-subject-model.mjs`가 고정 버전 모델 약 13.8MB를 다운로드하고 SHA-256을 검증합니다. 추론 런타임도 별도로 포함됩니다. 모델 파일은 Git에 저장하지 않습니다. 라이선스는 앱의 `dist/licenses`에 포함됩니다.

## v0.9.0 로컬 마스킹

- 오른쪽 **마스크** 탭에서 브러시·선형·원형 마스크를 추가하고 사진 위를 드래그합니다. 원형은 중심에서 바깥쪽으로, 선형은 강하게 적용할 곳에서 약하게 적용할 곳으로 그립니다. 브러시는 여러 획을 누적할 수 있고 영역 전체를 지우고 다시 그릴 수 있습니다. Esc 또는 포인터 취소는 그리는 중인 획만 취소합니다.
- 마스크별 **노출·대비·채도·색온도**, 강도, 경계 부드러움, 반전, 켜기/끄기와 삭제를 지원합니다. 원형·선형은 다시 드래그하면 영역이 바뀝니다. 자동 피사체 선택이나 부분 지우개는 이번 버전에 포함하지 않습니다.
- 사진당 최대 8개, 브러시당 최대 1,024개 경로 지점입니다. 목록 위에서 아래로 순서대로 적용됩니다. 마스크 좌표를 원본에 고정해 자르기·90도 회전·뒤집기 이후에도 같은 피사체 영역에 적용합니다.
- 기존 8비트와 고정밀/HDR 엔진, 원본 비교, 실행 취소/다시 실행, 자동 저장, 프로젝트와 모든 내보내기에 연결됩니다. 고정밀 경로는 로컬 보정도 선형 부동소수점으로 합성해 HDR 범위를 유지합니다. 붉은 선택 영역 표시는 출력되지 않습니다.
- 전체 프리셋은 로컬 마스크를 유지하며 **모든 보정 초기화**는 마스크도 제거합니다. 초기화는 실행 취소할 수 있습니다. 새 프로젝트는 **형식 버전 3**이므로 v0.9.0 이상에서 열어야 하며, v1/v2 프로젝트는 빈 마스크 목록으로 읽습니다.
...
- macOS Core Image와 Windows LibRaw 경로 모두 새 RAW를 처음부터 Display P3로 현상합니다. 이미 sRGB로 현상된 프로젝트는 **RAW 원본에서 16비트 P3 다시 현상**을 눌러 넓은 색역을 다시 가져올 수 있습니다. 현재 보정값/별점/원본 RAW는 유지하고, 실행 취소 기록은 재현상 결과부터 새로 시작합니다.
- 기존 sRGB JPEG를 P3로 바꾸는 것만으로 잃어버린 색이 복구되지는 않습니다. RAW나 P3 원본이 필요합니다. P3 결과를 충실히 표시하려면 P3를 지원하고 색상 관리가 설정된 화면이 필요합니다.
- v0.8.0부터 고정밀/HDR 처리를 지원합니다(위 항목 참조). 영화용 DCI-P3 감마 2.6은 지원하지 않습니다. 두 운영체제의 RAW 엔진 현상 결과는 다를 수 있습니다.
...
- Windows RAW 처리는 별도 코덱 설치 없이 로컬 Worker에서 실행합니다. 카메라 화이트밸런스, 전체 해상도, 16비트 Display P3 출력(v0.8.0부터)이며 120초 제한과 작업 후 Worker 정리를 적용합니다. macOS는 기존 Core Image 엔진을 유지하므로 새 RAW를 열었을 때 두 엔진의 색/크롭 결과가 다를 수 있습니다.
...
- `CIRAWFilter.outputImage`로 센서 데이터를 전체 해상도로 현상합니다. 내장 JPEG 미리보기로 대체하지 않습니다. v0.8.0부터 **16비트 Display P3 작업 이미지**에서 부동소수점 편집과 16비트 출력을 지원합니다. 센서 선형 데이터에서 다시 계산하는 노출/화이트밸런스 편집은 지원하지 않습니다.
...
- 새 RAW는 **파일당 120MB / 32MP 이하**입니다. 현상 시간은 최대 120초이며 지원하지 않거나 손상된 파일은 오류를 표시하고 다른 사진의 편집을 유지합니다. RAW와 현상 PNG를 함께 저장하므로 메모리와 프로젝트 파일 용량이 증가합니다.
...
- sRGB / Display P3 부동소수점 보정과 기존 8비트 호환 경로를 제공합니다. Lightroom의 RAW/색상 처리 엔진과 동일하지 않습니다.
- HEIC/TIFF, LUT 기반 ICC 고정밀 입력, AI 피사체 마스크, 자유 영역 자르기, 렌즈 보정, 일괄 내보내기는 미지원입니다.
- 일반 사진당 80MB(기존 8비트 60MP / 고정밀 32MP), 새 RAW 120MB / 32MP, 작업 공간당 최대 200장입니다. 실제 수용량은 메모리와 로컬 저장 공간에 따라 달라집니다.
